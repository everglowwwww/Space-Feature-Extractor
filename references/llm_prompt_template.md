...
本工具处理的平面图可能是手绘草图，请按以下约定识别各建筑元素：

- **粗实线（或双线）**= 实体墙，空间的物理围护边界
- **弧线**= 门的开启方向（单扇门或双扇门）
- **双线/三线（位于墙体上）**= 窗户
- **细实线**= 家具轮廓（桌、椅、沙发、柜等）
- **虚线或无线条的裁切边缘**= 非墙边界，仅表示功能区域的逻辑分界线，该侧无物理围护

特别注意：当平面图中某些边没有画墙线（呈开放状态），说明该空间在那个方向上是开放的或与相邻区域连通的。请据此判断围合度（enclosure_ratio），而不是假设所有边都有墙。
...
### 围护结构
- `enclosure_ratio`：0 = 完全开放（如广场），0.5 = 半开放（三面墙），1 = 完全封闭（四面墙+门关上）。请根据平面图中实际画出的墙线来判断，没有墙线的边视为开放。
- `window_wall_ratio`：窗面积 / 围护面总面积。住宅 0.15-0.25，办公 0.30-0.50，商业 0.50-0.70。**围合度联动规则：当 enclosure_ratio < 0.5 时，围护面不完整，window_wall_ratio 必须填 -1（无意义）；当 0.5 ≤ enclosure_ratio < 0.8 时，仍正常估算 WWR，但仅基于实际存在的围护面计算；当 enclosure_ratio ≥ 0.8 时，WWR 完全有效。**
- `perimeter_m`：空间平面轮廓周长（包含所有边，无论是否有墙）
